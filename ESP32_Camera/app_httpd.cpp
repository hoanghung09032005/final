#line 1 "E:\\Phase 2\\ESP32_Camera\\app_httpd.cpp"
...
#endif
    /* SỬA: bản gốc Espressif gọi log_i(...) Ở ĐÂY, tức MỖI KHUNG HÌNH video
     * (15-30 lần/giây, suốt thời gian đang stream) - trên ESP32-S3, log_i()
     * mặc định in ra CÙNG cổng USB-CDC với đối tượng "Serial" dùng trong
     * car_bridge.cpp. Nếu không có Serial Monitor đọc phía USB (trường hợp
     * chạy pin thực tế), việc in liên tục với tần suất này dễ làm nghẽn/
     * chặn USB-CDC hơn NHIỀU so với phần telemetry ít ỏi của STM32 - khả
     * năng cao đây mới là nguyên nhân chính gây Task Watchdog reset ->
     * "connection was forcibly closed" phía PC, không phải car_bridge.cpp.
     * Đổi sang IN THƯA (mỗi STREAM_LOG_EVERY_N_FRAMES khung mới in 1 lần)
     * thay vì bỏ hẳn, để vẫn xem được FPS lúc cần debug qua Serial Monitor
     * mà không spam mỗi frame. Đặt macro = 1 để quay lại hành vi log mỗi
     * khung như bản gốc nếu cần debug sát sao (chỉ nên bật lúc có cắm dây
     * USB + mở Serial Monitor, không bật khi chạy pin không giám sát). */
#define STREAM_LOG_EVERY_N_FRAMES  120   /* ~mỗi 4-8 giây tuỳ fps thật */
    static uint32_t stream_frame_count = 0;
    if ((++stream_frame_count % STREAM_LOG_EVERY_N_FRAMES) == 0) {
      log_i(
